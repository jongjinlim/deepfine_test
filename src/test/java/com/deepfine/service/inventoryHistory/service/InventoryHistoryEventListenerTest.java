...
class InventoryHistoryEventListenerTest {
    ...
    /**
     * Feature: 재고 입고 이력 기록 테스트
     * Background
     *     Given SPRING_ONION 이라는 이름의 재고는 아직 등록되어 있지 않음
     * <p>
     * Scenario: 50개 입고 요청함
     * <p>
     * Then 트랜잭션 커밋 후 INVENTORY_HISTORY에 INCREASE 이력이 정확히 남아야 함
     */
    @Test
    ...
    /**
     * Feature: 재고 출고 이력 기록 테스트
     * Background
     *     Given KURLY_001 이라는 이름의 재고 100개가 등록되어 있음
     * <p>
     * Scenario: 30개 출고 요청함
     * <p>
     * Then 트랜잭션 커밋 후 INVENTORY_HISTORY에 DECREASE 이력이 정확히 남아야 함
     */
    @Test
    void decreaseHistoryTest() {
        Inventory inventory = inventoryService.increase("KURLY_001", 100);
        inventoryId = inventory.getInventoryId();
